...
IdentityOperator2D<T, Basis2D>::IdentityOperator2D(const Basis2D &_basis2d, T _entrybound,
...
template <typename T, typename Basis2D>
T
IdentityOperator2D<T, Basis2D>::operator()(const Index2D &row_index, const Index2D &col_index)
{
...
template <typename T, typename Basis2D>
T
IdentityOperator2D<T, Basis2D>::prec(const Index2D &index)
{
    return 1.;
}
