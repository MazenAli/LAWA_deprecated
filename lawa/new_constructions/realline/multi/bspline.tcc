#ifndef LAWA_CONSTRUCTIONS_REALLINE_MULTI_BSPLINE_TCC
#define LAWA_CONSTRUCTIONS_REALLINE_MULTI_BSPLINE_TCC 1
...
template <typename T>
BSpline<T,Orthogonal,R,Multi>::BSpline(const unsigned int _d)
    : d(_d)
{
    assert(d>=2);
...
template <typename T>
T
BSpline<T,Orthogonal,R,Multi>::operator()(T x, int j, Integer k, unsigned short deriv) const
{
    const int type = _type(k);
    const Integer shift = _shift(k);
...
template <typename T>
Support<T>
BSpline<T,Orthogonal,R,Multi>::support(int j, Integer k) const
{
    const int type = _type(k);
    const Integer shift = _shift(k);
...
template <typename T>
DenseVector<Array<T> >
BSpline<T,Orthogonal,R,Multi>::singularSupport(int j, Integer k) const
{
...
template <typename T>
Integer
BSpline<T,Orthogonal,R,Multi>::_shift( Integer  k) const
{
...
template <typename T>
int
BSpline<T,Orthogonal,R,Multi>::_type( Integer  k) const
{
...
#endif // LAWA_CONSTRUCTIONS_REALLINE_MULTI_BSPLINE_TCC
