#ifndef LAWA_CONSTRUCTIONS_INTERVAL_PRIMBS_DUAL_BASIS_TCC
#define LAWA_CONSTRUCTIONS_INTERVAL_PRIMBS_DUAL_BASIS_TCC 1

#include <cassert>

namespace lawa {

template <typename T>
Basis<T,Dual,Interval,Primbs>::Basis(unsigned int _d, unsigned int _d_, int j)
    : mra(_d, j), mra_(_d, _d_, j),
      d(_d), d_(_d_), mu(d&1),
      min_j0(mra_.min_j0), j0(mra_.j0), _bc(2,0), _j(j0), psi_(*this)
{
    _calcM1_();
}

template <typename T>
int
Basis<T,Dual,Interval,Primbs>::level() const
{
    return _j;
}

template <typename T>
void
Basis<T,Dual,Interval,Primbs>::setLevel(int j) const
{
    if (j!=_j) {
        assert(j>=min_j0);
        _j = j;
        mra.setLevel(_j);
        mra_.setLevel(_j);
        M1_.setLevel(_j);
    }
}

template <typename T>
template <BoundaryCondition BC>
void
Basis<T,Dual,Interval,Primbs>::enforceBoundaryCondition()
{
    typedef flens::GeMatrix<flens::FullStorage<T,cxxblas::ColMajor> > 
            FullColMatrix;

    BSpline<T,Primal,R,CDF> phiR(d);

    if ((_bc(0)==0) && (_bc(1)==0)) {
        setLevel(min_j0);
        _bc(0) = _bc(1) = 1;
        mra.enforceBoundaryCondition<BC>();
        mra_.enforceBoundaryCondition<BC>();

        int j = min_j0;
        FullColMatrix H(pow2i<T>(j+1)-d+1,
                        pow2i<T>(j+1)-d+1), 
                      Tmp, Tmp2, Tmp3, Tmp4, Mj1, Mjj1;
        H.diag(0) = 1.;

        DenseVector<Array<T> > x(pow2i<T>(j+1)-d+1), tmp;
        for (int i=1; i<=d+1; ++i) {
            x(i) = phiR.a(phiR.a.firstIndex()-1+i);
        }

        for (int i=1; i<=d; ++i) {
            FullColMatrix A(pow2i<T>(j+1)-d+1,
                             pow2i<T>(j+1)-d+1);
            A.diag(0) = 1.;
            if (i-2*ifloor<int>(i/2.)==1) {
                for (Integer k=iceil<Integer>(1/2.-(i+1)/4.); k<=ifloor<Integer>(pow2i<Integer>(j)-d/2.-(i+1)/4.); ++k) {
                    A((i+1)/2.+2*k,(i+1)/2.+2*k+1) = -x(ifloor<int>(i/2.)+1) / x(ifloor<int>(i/2.)+2);
                }
                tmp = x;
                x = A*tmp;
                blas::mm(NoTrans,NoTrans,1.,A,H,0.,Tmp);
                H = Tmp;
            } else if (i-2*ifloor<int>(i/2.)==0) {
                for (Integer k=iceil<Integer>(1/2.-i/4.); k<=ifloor<Integer>(pow2i<Integer>(j)-d/2.-i/4.); ++k) {
                    A(pow2i<T>(j+1)-d+2-i/2.-2*k,pow2i<T>(j+1)-d+1-i/2.-2*k) = -x(d-i/2.+2)/x(d-i/2.+1);
                }
                tmp = x;
                x = A*tmp;
                blas::mm(NoTrans,NoTrans,1.,A,H,0.,Tmp);
                H = Tmp;
            }
        }
        T b = 0;
        for (int i=1; i<=pow2i<Integer>(j+1)-d+1; ++i) {
            b += x(i);
        }

        FullColMatrix Hj(pow2i<T>(j+1)+d-3,
                          pow2i<T>(j+1)+d-3), InvHj;
        Hj.diag(0) = 1.;
        for (Integer n=1; n<=pow2i<Integer>(j+1)-d+1; ++n) {
            for (Integer m=1; m<=pow2i<Integer>(j+1)-d+1; ++m) {
                Hj(d-2+n,d-2+m) = H(n,m);
            }
        }
        InvHj = inv(Hj);

        FullColMatrix F(pow2i<Integer>(j+1)-d+1,pow2i<Integer>(j)-d+1);
        for (Integer k=1; k<=pow2i<Integer>(j)-d+1; ++k) {
            F(iceil(d/2.)+2*k-2,k) = 1;
        }

        FullColMatrix Fj(pow2i<Integer>(j+1)+d-3,pow2i<Integer>(j));
        for (Integer k=1; k<=ifloor<Integer>(d/2.); ++k) {
            Fj(d-2+k,k) =1;
            Fj(pow2i<Integer>(j+1)-k,pow2i<Integer>(j)+1-k) = 1;
        }
        for (Integer n=1; n<=pow2i<Integer>(j+1)-d+1; ++n) {
            for (Integer m=1; m<=pow2i<Integer>(j)-d+1; ++m) {
                Fj(d-2+n,iceil(d/2.)-1+m) = F(n,m);
            }
        }

        FullColMatrix Pj(pow2i<Integer>(j+1)+d-3,
                          pow2i<Integer>(j+1)+d-3), 
                       Identity(pow2i<Integer>(j+1)+d-3,
                                pow2i<Integer>(j+1)+d-3);
        Pj.diag(0) = 1.;
        for (int n=1; n<=2*(d-2)+1; ++n) {
            for (int m=1; m<=d-2; ++m) {
                Pj(n,m) = mra_.MP(n+1,m+1);
                Pj(pow2i<Integer>(j+1)+d-2-n,pow2i<Integer>(j+1)+d-2-m) = mra_.MP(n+1,m+1);
            }
        }

        blas::mm(NoTrans,NoTrans,1.,Pj,InvHj,0.,Tmp);
        blas::mm(NoTrans,NoTrans,1.,Tmp,Fj,0.,Tmp2);

        Identity.diag(0) = 1.;
        FullColMatrix DM0, DM0_;
        densify(cxxblas::NoTrans, mra.M0, DM0);
        densify(cxxblas::NoTrans, mra_.M0_, DM0_);

        T factor = powii(-1,d+1)*1./b;
        blas::mm(NoTrans,Trans,1.,DM0,DM0_,0.,Tmp3);
        Tmp4 = Identity;
        blas::axpy(NoTrans,-1.,Tmp3,Tmp4);
        blas::mm(NoTrans,NoTrans,factor,Tmp4,Tmp2,0.,Mj1);

        blas::mm(Trans,NoTrans,1.,Fj,Hj,0.,Tmp);
        blas::mm(NoTrans,NoTrans,1.,Tmp,inv(Pj),0.,Tmp2);

        Mjj1.engine().resize(Tmp2.cols(),Tmp2.rows());
        for (Integer row=Mjj1.firstRow(); row<=Mjj1.lastRow(); ++row) {
            for (Integer col=Mjj1.firstCol(); col<=Mjj1.lastCol(); ++col) {
                Mjj1(row,col) = 2./factor*Tmp2(col,row); 
            }
        }

        if ((d/2.)>ifloor<int>(d/2.)) {
            FullColMatrix Nj1(pow2i<Integer>(j+1)+d-3,pow2i<Integer>(j-1)),
            Nj1tr(pow2i<Integer>(j+1)+d-3,pow2i<Integer>(j-1)),
            Njj1(pow2i<Integer>(j+1)+d-3,pow2i<Integer>(j-1)),
            Njj1tr(pow2i<Integer>(j+1)+d-3,pow2i<Integer>(j-1));
            for (Integer n=1; n<=pow2i<Integer>(j+1)+d-3; ++n) {
                for (Integer m=1; m<=pow2i<Integer>(j-1); ++m) {
                    Nj1(n,m) = Mj1(n,m);
                    Nj1tr(pow2i<Integer>(j+1)+d-2-n,pow2i<Integer>(j-1)+1-m) = Nj1(n,m);
                    Njj1(n,m) = Mjj1(n,m);
                    Njj1tr(pow2i<Integer>(j+1)+d-2-n,pow2i<Integer>(j-1)+1-m) = Njj1(n,m);
                }
            }

            FullColMatrix Hj1(pow2i<Integer>(j+1)+d-3,pow2i<Integer>(j)),
            Hjj1(pow2i<Integer>(j+1)+d-3,pow2i<Integer>(j));
            for (Integer n=1; n<=pow2i<Integer>(j+1)+d-3; ++n) {
                for (Integer m=1; m<=pow2i<Integer>(j-1); ++m) {
                    Hj1(n,m) = Nj1(n,m);
                    Hjj1(n,m) = Njj1(n,m);
                }
                for (Integer m=pow2i<Integer>(j-1)+1; m<=pow2i<Integer>(j); ++m) {
                    Hj1(n,m) = Nj1tr(n,m-pow2i<Integer>(j-1));
                    Hjj1(n,m) = Njj1tr(n,m-pow2i<Integer>(j-1));
                }
            }

            FullColMatrix Kj, InvKj;
            blas::mm(Trans,NoTrans,1.,Hjj1,Hj1,0.,Kj);
            InvKj = inv(Kj);
            blas::mm(NoTrans,NoTrans,2.,Hj1,InvKj,0.,Mj1);
            Mjj1 = Hjj1;
        }

        int numPrimalCols = (d+d_)/2-1,
            numDualCols = (d+1)/2-1;

        if (d==2) {
            numPrimalCols += 1;
            numDualCols += 1;
        }

        blas::scal(rsqrt2<T>()*.5, Mjj1);
        M1_ = RefinementMatrix<T,Interval,Primbs>(numDualCols, numDualCols, Mjj1, min_j0, min_j0);
        setLevel(_j);
    }
}

template <typename T>
const BasisFunction<T,Dual,Interval,Primbs> &
Basis<T,Dual,Interval,Primbs>::generator(XType xtype) const
{
    if (xtype==XBSpline) {
        return mra.phi_;
    } else {
        return psi_;
    }
}

// cardinalities of whole, left, inner, right index sets (primal).
template <typename T>
Integer
Basis<T,Dual,Interval,Primbs>::cardJ_(int j) const
{
    assert(0);
    assert(j>=min_j0);
    return pow2i<Integer>(j);
}

template <typename T>
Integer
Basis<T,Dual,Interval,Primbs>::cardJ_L(int j) const
{
    assert(0);
    assert(j>=min_j0);
    return d + d_ - 2;
}

template <typename T>
Integer
Basis<T,Dual,Interval,Primbs>::cardJ_I(int j) const
{
    assert(0);
    assert(j>=min_j0);
    return pow2i<Integer>(j) - 2*(d + d_ - 2);
}

template <typename T>
Integer
Basis<T,Dual,Interval,Primbs>::cardJ_R(int j) const
{
    assert(0);
    assert(j>=min_j0);
    return d + d_ - 2;
}

// ranges of whole, left, inner, right index sets (primal).
template <typename T>
const Range<Integer>
Basis<T,Dual,Interval,Primbs>::rangeJ_(int j) const
{
    assert(0);
    assert(j>=min_j0);
    return _(1,pow2i<Integer>(j));
}

template <typename T>
const Range<Integer>
Basis<T,Dual,Interval,Primbs>::rangeJ_L(int j) const
{
    assert(0);
    assert(j>=min_j0);
    return _(1,d+d_-2);
}

template <typename T>
const Range<Integer>
Basis<T,Dual,Interval,Primbs>::rangeJ_I(int j) const
{
    assert(0);
    assert(j>=min_j0);
    return _(d+d_-1,pow2i<Integer>(j)-(d+d_-3));
    
}

template <typename T>
const Range<Integer>
Basis<T,Dual,Interval,Primbs>::rangeJ_R(int j) const
{
    assert(0);
    assert(j>=min_j0);
    return _(pow2i<Integer>(j)-(d+d_-2),pow2i<Integer>(j));

}

template <typename T>
void
Basis<T,Dual,Interval,Primbs>::_calcM1_()
{
    typedef flens::GeMatrix<flens::FullStorage<T,cxxblas::ColMajor> > FullColMatrix;
    BSpline<T,Primal,R,CDF> phiR(d);

    setLevel(min_j0);
    FullColMatrix H(pow2i<T>(min_j0+1)-d+1,pow2i<T>(min_j0+1)-d+1), 
                   Tmp, Tmp2, Tmp3, Tmp4, Mj1, Mjj1;
    H.diag(0) = 1.;

    DenseVector<Array<T> > x(pow2i<T>(min_j0+1)-d+1), tmp;
    for (int i=1; i<=d+1; ++i) {
        x(i) = phiR.a(phiR.a.firstIndex()-1+i);
    }
    for (int i=1; i<=d; ++i) {
        FullColMatrix A(pow2i<T>(min_j0+1)-d+1,pow2i<T>(min_j0+1)-d+1);
        A.diag(0) = 1.;
        if (i-2*ifloor<int>(i/2.)==1) {
            for (Integer k=iceil<Integer>(1/2.-(i+1)/4.); k<=ifloor<Integer>(pow2i<T>(min_j0)-d/2.-(i+1)/4.); ++k) {
                A((i+1)/2.+2*k,(i+1)/2.+2*k+1) = -x(ifloor<int>(i/2.)+1) / x(ifloor<int>(i/2.)+2);
            }
            tmp = x;
            x = A*tmp;
            blas::mm(NoTrans,NoTrans,1.,A,H,0.,Tmp);
            H = Tmp;
        } else if (i-2*ifloor<int>(i/2.)==0) {
            for (Integer k=iceil<Integer>(1/2.-i/4.); k<=ifloor<Integer>(pow2i<T>(min_j0)-d/2.-i/4.); ++k) {
                A(pow2i<T>(min_j0+1)-d+2-i/2.-2*k,pow2i<T>(min_j0+1)-d+1-i/2.-2*k) = -x(d-i/2.+2)/x(d-i/2.+1);
            }
            tmp = x;
            x = A*tmp;
            blas::mm(NoTrans,NoTrans,1.,A,H,0.,Tmp);
            H = Tmp;
        }
    }

    T b = 0;
    for (Integer i=1; i<=pow2i<T>(min_j0+1)-d+1; ++i) {
        b += x(i);
    }

    FullColMatrix Hj(pow2i<T>(min_j0+1)+d-1,pow2i<T>(min_j0+1)+d-1), InvHj;
    Hj.diag(0) = 1.;
    for (Integer n=1; n<=pow2i<T>(min_j0+1)-d+1; ++n) {
        for (Integer m=1; m<=pow2i<T>(min_j0+1)-d+1; ++m) {
            Hj(d-1+n,d-1+m) = H(n,m);
        }
    }
    InvHj = inv(Hj);

    FullColMatrix F(pow2i<T>(min_j0+1)-d+1,pow2i<T>(min_j0)-d+1);
    for (Integer k=1; k<=pow2i<T>(min_j0)-d+1; ++k) {
        F(iceil(d/2.)+2*k-2,k) = 1;
    }

    FullColMatrix Fj(pow2i<T>(min_j0+1)+d-1,pow2i<T>(min_j0));
    for (Integer k=1; k<=iceil((d-1)/2.); ++k) {
        Fj(d-1+k,k) =1;
        Fj(pow2i<T>(min_j0+1)+1-k,pow2i<T>(min_j0)+1-k) = 1;
    }
    for (Integer n=1; n<=pow2i<T>(min_j0+1)-d+1; ++n) {
        for (Integer m=1; m<=pow2i<T>(min_j0)-d+1; ++m) {
            Fj(d-1+n,ifloor<int>((d-1)/2.+m)) = F(n,m);
        }
    }

    FullColMatrix Pj(pow2i<T>(min_j0+1)+d-1,pow2i<T>(min_j0+1)+d-1),
                   Identity(pow2i<T>(min_j0+1)+d-1,pow2i<T>(min_j0+1)+d-1);
    Pj.diag(0) = 1.;
    for (int n=1; n<=2*(d-1); ++n) {
        for (int m=1; m<=d-1; ++m) {
            Pj(n,m) = mra_.MP(n,m);
            Pj(pow2i<T>(min_j0+1)+d-n,pow2i<T>(min_j0+1)+d-m) = mra_.MP(n,m);
        }
    }

    blas::mm(NoTrans,NoTrans,1.,Pj,InvHj,0.,Tmp);
    blas::mm(NoTrans,NoTrans,1.,Tmp,Fj,0.,Tmp2);
    Identity.diag(0) = 1.;

    FullColMatrix DM0, DM0_;
    densify(cxxblas::NoTrans, mra.M0, DM0);
    densify(cxxblas::NoTrans, mra_.M0_, DM0_);

    T factor = powii(-1,d+1)*2./b;
    blas::mm(NoTrans,Trans,1.,DM0,DM0_,0.,Tmp3);
    Tmp4 = Identity;
    blas::axpy(NoTrans,-1.,Tmp3,Tmp4);
    blas::mm(NoTrans,NoTrans,factor,Tmp4,Tmp2,0.,Mj1);

    blas::mm(Trans,NoTrans,1.,Fj,Hj,0.,Tmp);
    blas::mm(NoTrans,NoTrans,1.,Tmp,inv(Pj),0.,Tmp2);

//    blas::axpy(Trans,2./factor,Tmp2,Mjj1);
    Mjj1.engine().resize(Tmp2.cols(),Tmp2.rows());
    for (Integer row=Mjj1.firstRow(); row<=Mjj1.lastRow(); ++row) {
        for (Integer col=Mjj1.firstCol(); col<=Mjj1.lastCol(); ++col) {
            Mjj1(row,col) = 2./factor*Tmp2(col,row); 
        }
    }

    if ((d/2.)>ifloor<int>(d/2.)) {
        FullColMatrix Nj1(pow2i<T>(min_j0+1)+d-1,pow2i<T>(min_j0-1)),
                      Nj1tr(pow2i<T>(min_j0+1)+d-1,pow2i<T>(min_j0-1)),
                      Njj1(pow2i<T>(min_j0+1)+d-1,pow2i<T>(min_j0-1)),
                      Njj1tr(pow2i<T>(min_j0+1)+d-1,pow2i<T>(min_j0-1));
        for (Integer n=1; n<=pow2i<T>(min_j0+1)+d-1; ++n) {
            for (Integer m=1; m<=pow2i<T>(min_j0-1); ++m) {
                Nj1(n,m) = Mj1(n,m);
                Nj1tr(pow2i<T>(min_j0+1)+d-n,pow2i<T>(min_j0-1)+1-m) = Nj1(n,m);
                Njj1(n,m) = Mjj1(n,m);
                Njj1tr(pow2i<T>(min_j0+1)+d-n,pow2i<T>(min_j0-1)+1-m) = Njj1(n,m);
            }
        }

        FullColMatrix Hj1(pow2i<T>(min_j0+1)+d-1,pow2i<T>(min_j0)),
                      Hjj1(pow2i<T>(min_j0+1)+d-1,pow2i<T>(min_j0));
        for (Integer n=1; n<=pow2i<T>(min_j0+1)+d-1; ++n) {
            for (Integer m=1; m<=pow2i<T>(min_j0-1); ++m) {
                Hj1(n,m) = Nj1(n,m);
                Hjj1(n,m) = Njj1(n,m);
            }
            for (Integer m=pow2i<T>(min_j0-1)+1; m<=pow2i<T>(min_j0); ++m) {
                Hj1(n,m) = Nj1tr(n,m-pow2i<T>(min_j0-1));
                Hjj1(n,m) = Njj1tr(n,m-pow2i<T>(min_j0-1));
            }
        }

        FullColMatrix Kj, InvKj;
        blas::mm(Trans,NoTrans,1.,Hjj1,Hj1,0.,Kj);
        InvKj = inv(Kj);
        blas::mm(NoTrans,NoTrans,2.,Hj1,InvKj,0.,Mj1);
        Mjj1 = Hjj1;
    }

    int numPrimalCols = d-1 + (d_-d)/2,
        numDualCols = (d+1)/2-1;

    if (d==2) {
        numPrimalCols += 1;
        numDualCols += 1;
    }

    blas::scal(rsqrt2<T>()*.5, Mjj1);
    M1_ = RefinementMatrix<T,Interval,Primbs>(numDualCols,numDualCols,Mjj1,min_j0,min_j0);
    setLevel(_j);
}


} // namespace lawa

#endif // LAWA_CONSTRUCTIONS_INTERVAL_PRIMBS_DUAL_BASIS_TCC
