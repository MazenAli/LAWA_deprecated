#ifndef LAWA_CONSTRUCTIONS_INTERVAL_PRIMBS_DUAL_MRA_TCC
#define LAWA_CONSTRUCTIONS_INTERVAL_PRIMBS_DUAL_MRA_TCC 1
...
{
    assert(j>=min_j0);
    return pow2i<Integer>(j) + d - 1 - (_bc(0)+_bc(1));
}
...
{
    assert(j>=min_j0);
    return pow2i<Integer>(j) + d - 1 - 2*(d+d_-2);
}
...
{
    assert(j>=min_j0);
    return Range<Integer>(d+d_-1, pow2i<Integer>(j)+d-1-(d+d_-2));
}
...
{
    assert(j>=min_j0);
    return Range<Integer>(pow2i<T>(j)+d-1-(d+d_-2)+1, pow2i<T>(j)+d-1-_bc(1));
}
...
//    TRD(_(1,d+d_-2),_(1,d+d_-2)) = transpose(inv(TR));
    ...
    int row = d+d_-1;
    for (Integer c=MDDD.lastCol()+1; c<=Mj0_.lastCol()-MDDD.numCols()+1; ++c,row+=2) {
...
{
    Integer n = L3.numRows();
    Integer m = L3.numCols();

    GeMatrix<FullStorage<T,ColMajor> > L2(n-m,m), LL2(n-m,m), L22, C(m,m);

    for (Integer j=1; j<=n-m; ++j) {
    ...
    for (Integer r=L2.firstRow(); r<=L2.lastRow(); ++r) {
        for (Integer c=L2.firstCol(); c<=L2.lastCol(); ++c) {
            L22(c,r) = L2(r,c);
        }
    }
...
MRA<T,Dual,Interval,Primbs>::_calcM0_()
{
    ...
    int row = d+d_-1;
    for (Integer c=MDDD.lastCol()+1; c<=Mj0_.lastCol()-MDDD.numCols(); ++c,row+=2) {
        Mj0_(_(row,row+phi_R.a_.length()-1),c) = factor*phi_R.a_;
    }
    arrow(Bugfix,Mj0_Right);
    Mj0_(_(Mj0_.lastRow()-Mj0_Right.numRows()+1,Mj0_.lastRow()),
...
#endif // LAWA_CONSTRUCTIONS_INTERVAL_PRIMBS_DUAL_MRA_TCC
