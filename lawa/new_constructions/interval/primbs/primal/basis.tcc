#ifndef LAWA_CONSTRUCTIONS_INTERVAL_PRIMBS_PRIMAL_BASIS_TCC
#define LAWA_CONSTRUCTIONS_INTERVAL_PRIMBS_PRIMAL_BASIS_TCC 1
    ...
    if ((_bc(0)==0) && (_bc(1)==0)) {
        ...
        for (int i=1; i<=d; ++i) {
            ...
            if (i-2*ifloor<int>(i/2.)==1) {
                for (Integer k=iceil<Integer>(1/2.-(i+1)/4.); k<=ifloor<Integer>(pow2i<T>(j)-d/2.-(i+1)/4.); ++k) {
                ...
                for (Integer k=iceil<Integer>(1/2.-i/4.); k<=ifloor<Integer>(pow2i<T>(j)-d/2.-i/4.); ++k) {
        ...
        T b = 0;
        for (Integer i=1; i<=pow2i<T>(j+1)-d+1; ++i) {
            b += x(i);
        }
        ...
        Hj.diag(0) = 1.;
        for (Integer n=1; n<=pow2i<T>(j+1)-d+1; ++n) {
            for (Integer m=1; m<=pow2i<T>(j+1)-d+1; ++m) {
        ...
        for (Integer k=1; k<=pow2i<T>(j)-d+1; ++k) {
            F(iceil(d/2.)+2*k-2,k) = 1;
        }

        FullColMatrix Fj(pow2i<T>(j+1)+d-3,pow2i<T>(j));
        for (Integer k=1; k<=ifloor(d/2.); ++k) {
            Fj(d-2+k,k) =1;
            Fj(pow2i<T>(j+1)-k,pow2i<T>(j)+1-k) = 1;
        }
        for (Integer n=1; n<=pow2i<T>(j+1)-d+1; ++n) {
            for (Integer m=1; m<=pow2i<T>(j)-d+1; ++m) {
        ...
        if ((d/2.)>ifloor<int>(d/2.)) {
            ...
            for (Integer n=1; n<=pow2i<T>(j+1)+d-3; ++n) {
                for (Integer m=1; m<=pow2i<T>(j-1); ++m) {
                    Nj1(n,m) = Mj1(n,m);
            ...
            for (Integer n=1; n<=pow2i<T>(j+1)+d-3; ++n) {
                ...
                for (Integer m=pow2i<T>(j-1)+1; m<=pow2i<T>(j); ++m) {
...
//    blas::axpy(Trans,2./factor,Tmp2,Mjj1);
    ...
    Integer numPrimalCols = d-1 + (d_-d)/2,
         numDualCols = (d+1)/2-1;
...
#endif // LAWA_CONSTRUCTIONS_INTERVAL_PRIMBS_PRIMAL_BASIS_TCC
