#ifndef LAWA_CONSTRUCTIONS_INTERVAL_PRIMBS_PRIMAL_MRA_TCC
#define LAWA_CONSTRUCTIONS_INTERVAL_PRIMBS_PRIMAL_MRA_TCC 1
...
{
    return Range<Integer>(1 + _bc(0), d - 1);
}
...
{
    assert(j>=min_j0);
    return Range<Integer>(d, pow2i<T>(j));
}
...
{
    assert(j>=min_j0);
    return Range<Integer>(pow2i<T>(j) + 1, pow2i<T>(j) + d - 1 - _bc(1));
}
...
MRA<T,Primal,Interval,Primbs>::_calcM0()
{
    ...
    FullColMatrix R(_(1-l2, pow2i<int>(min_j0)-l1-1),
                    _(1-l2, pow2i<int>(min_j0)-l1-1));
    R.diag(0) = 1.;
    ...
    FullColMatrix ExtM0(_(-l2+1,pow2i<int>(min_j0+1)-l1-1),
                        _(-l2+1,pow2i<int>(min_j0)-l1-1));
    ...
    FullColMatrix RjPlus1(_(-l2+1,pow2i<int>(min_j0+1)-l1-1),
                          _(-l2+1,pow2i<int>(min_j0+1)-l1-1));
    RjPlus1.diag(0) = 1.;
...
#endif // LAWA_CONSTRUCTIONS_INTERVAL_PRIMBS_PRIMAL_MRA_TCC
