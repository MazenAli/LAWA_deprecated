...
#define LAWA_CONSTRUCTIONS_INTERVAL_DIJKEMA_DUAL_MRA_TCC 1
...
{
    assert(j>=min_j0);
    return Range<Integer>(1 + _bc(0), pow2i<Integer>(j) + d - 1 - _bc(1));
}
...
{
    return Range<Integer>(1 + _bc(0), d + d_ - 2);
}
...
{
    assert(j>=min_j0);
    return Range<Integer>(d + d_ - 1, pow2i<Integer>(j) - d_ + 1);
}
...
{
    assert(j>=min_j0);
    return Range<Integer>(pow2i<Integer>(j) - d_ + 2, pow2i<Integer>(j) + d - 1 - _bc(1));
}
...
MRA<T,Dual,Interval,Dijkema>::_calcM0_()
{
    ...
    typedef DenseVector<Array<Integer> > IntegerVector;
    ...
    for (Integer k=A.firstRow(); k<=A.lastRow(); ++k) {
        for (Integer m=A.firstCol(); m<=A.lastCol(); ++m) {
    ...
    IntegerVector positions(r.length());
    for (Integer i=r.firstIndex(); i<=r.lastIndex(); ++i) {
        int j=0;
    ...
    if ((_bc(0)==1) && (_bc(1)==1)) {
        if ((d==3) && (d_==3)) {
            indices.engine().resize(3l);
            indices = 1, 2, 1;
        } else if ((d==3) && (d_==5)) {
            indices.engine().resize(5L);
            indices = 1, 3, 6, 8, 10;
        } else if ((d==3) && (d_==7)) {
            indices.engine().resize(7L);
            indices = 1, 2, 5, 8, 10, 12, 14;
        } else if ((d==3) && (d_==9)) {
            indices.engine().resize(9L);
            ...
            indices.engine().resize(11L);
            indices = 1, 2, 5, 8, 10, 12, 13, 16, 17, 18, 21;
        } else if ((d==5) && (d_==9)) {
            indices.engine().resize(11L);
            indices = 1, 2, 4, 7, 9, 12, 13, 15, 16, 17, 20;
        } else if ((d==2) && (d_==2)) {
            indices.engine().resize(2L);
            indices = 1, 1;
        } else if ((d==2) && (d_==4)) {
            indices.engine().resize(4L);
            indices = 1, 1, 4, 6;
        } else if ((d==2) && (d_==6)) {
            indices.engine().resize(6L);
            indices = 1, 6, 8, 12, 9, 11;
        } else if ((d==2) && (d_==8)) {
            indices.engine().resize(8L);
            indices = 1, 5, 4, 16, 11, 12, 9, 13;
        } else { // for d=2 we need all, for others not necessarily optimal!
            indices.engine().resize(d+d_-2L);
            for (Integer i=indices.firstIndex(); i<=indices.lastIndex(); ++i) {
                indices(i) = i;
            }
        }
    } else {
    ...
    FullColMatrix R_init(pow2i<int>(cons_j)+l2_-l1_-1,
                         pow2i<int>(cons_j)+l2-l1-1-_bc(0)-_bc(1),
                         -l2_+1, -l2+1+_bc(0));
    for (Integer c=R_init.firstCol(); c<=R_init.lastCol(); ++c) {
        R_init(c,c) = 1.;
    }
    ...
    Integer kmin = -l2+1, mmin = -l2_+1, pmin = -l2+1, qmin = -l2_+1,
              kmax = -l1-1, mmax = -l1_-1, pmax = -l1-1, qmax = -l1_-1;
    FullColMatrix C(_(pmin,pmax), _(qmin,qmax));
    Integer ZLength = C.numRows()*C.numCols();
    FullColMatrix Z(_(pmin*(qmax-qmin+1)+qmin,
                      pmin*(qmax-qmin+1)+qmin+ZLength-1),
                    _(pmin*(qmax-qmin+1)+qmin, 
                      pmin*(qmax-qmin+1)+qmin+ZLength-1));
    for (Integer p=pmin; p<=pmax; ++p) {
        for (Integer q=qmin; q<=qmax; ++q) {
            for (Integer k=kmin; k<=kmax; ++k) {
                for (Integer m=mmin; m<=mmax; ++m) {
    ...
    for (Integer i=Z.firstRow(); i<=Z.lastRow(); ++i) {
        Z(i,i) -= 1.;
    }
    Z *= -1;
    ...
    for (Integer p=pmin; p<=pmax; ++p) {
        for (Integer q=qmin; q<=qmax; ++q) {
            T sum = 0.;
            for (Integer k=std::max(-l2+1,l1+2*p); k<=l2+2*p; ++k) {
                for (Integer m=std::max(std::max(-l2_+1,l1_+2*q),l1-l2_+1+k);
    ...
    flens::DenseVector<Array<Integer> > pivots(f.length(),f.firstIndex());
    sv(Z,pivots,f);
    for (Integer p=pmin; p<=pmax; ++p) {
        for (Integer q=qmin; q<=qmax; ++q) {
    ...
    FullColMatrix ExtMass(pow2i<int>(cons_j)+d-1,
                          pow2i<int>(cons_j)+l2_-l1_-1,
                          -l2+1, -l2_+1);
    for (Integer r=ExtMass.firstRow(); r<=ExtMass.lastRow(); ++r) {
        ExtMass(r,r) = 1.;
    }
    ExtMass(C) = C;
    FullColMatrix CR;
    ...
    FullColMatrix R(_(1-l2, pow2i<int>(cons_j)-l1-1),
                    _(1-l2+_bc(0), pow2i<int>(cons_j)-l1-1-_bc(1)));
    Integer rr = R.firstRow()+_bc(0);
    for (Integer c=R.firstCol(); c<=R.lastCol(); ++c, ++rr) {
        R(rr,c) = 1.;
    }
    ...
    FullColMatrix ExtM0(_(-l2+1,pow2i<int>(cons_j+1)-l1-1),
                        _(-l2+1,pow2i<int>(cons_j)-l1-1));
    for (Integer q=ExtM0.firstCol(); q<=ExtM0.lastCol(); ++q) {
        for (Integer p=std::max(l1+2*q,ExtM0.firstRow());
             p<=std::min(l2+2*q,ExtM0.lastRow()); ++p) {
            ExtM0(p,q) = phi.a(p-2*q);
        }
    }

    FullColMatrix RjPlus1(_(-l2+1,pow2i<int>(cons_j+1)-l1-1),
                          _(-l2+1+_bc(0),pow2i<int>(cons_j+1)-l1-1-_bc(1)));
    rr = RjPlus1.firstRow()+_bc(0);
    for (Integer c=RjPlus1.firstCol(); c<=RjPlus1.lastCol(); ++c, ++rr) {
        RjPlus1(rr,c) = 1.;
    }
    if (d>2) {
    ...
    FullColMatrix ExtM0_(_(-l2_+1,pow2i<int>(cons_j+1)-l1_-1),
                         _(-l2_+1,pow2i<int>(cons_j)-l1_-1));
    for (Integer q=ExtM0_.firstCol(); q<=ExtM0_.lastCol(); ++q) {
        for (Integer p=std::max(l1_+2*q, ExtM0_.firstRow());
             p<=std::min(l2_+2*q, ExtM0_.lastRow()); ++p) {
            ExtM0_(p,q) = phi_R.a_(p-2*q);
        }
    }

    FullColMatrix ExtMassjPlus1(pow2i<int>(cons_j+1)+d-1, 
                                pow2i<int>(cons_j+1)+l2_-l1_-1,
                                -l2+1, -l2_+1);
    for (Integer r=ExtMassjPlus1.firstRow(); r<=ExtMassjPlus1.lastRow(); ++r) {
        ExtMassjPlus1(r,r) = 1.;
    }
    ExtMassjPlus1(C) = C;
