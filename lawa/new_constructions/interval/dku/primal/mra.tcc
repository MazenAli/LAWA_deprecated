#ifndef LAWA_CONSTRUCTIONS_INTERVAL_DKU_PRIMAL_MRA_TCC
#define LAWA_CONSTRUCTIONS_INTERVAL_DKU_PRIMAL_MRA_TCC 1
...
{
    assert(j>=min_j0);
    return Range<Integer>(l-d+_bc(0), pow2i<T>(j)-l+d-mu-_bc(1));
}
...
{
    return Range<Integer>(l-d+_bc(0), l-1);
}
...
{
    assert(j>=min_j0);
    return Range<Integer>(l, pow2i<T>(j)-q-1);
}
...
{
    assert(j>=min_j0);
    return Range<Integer>(pow2i<T>(j)-q, pow2i<T>(j)-q+d-1-_bc(1));
}
...
MRA<T,Primal,Interval,DKU>::_alpha_()
{
    ...
    for (int r=1; r<d; ++r) {
        // m<0
        for (Integer m=_Alpha_.firstRow(); m<0; ++m) {
            T tmp = 0.;
            for (int i=0; i<=r; ++i) {
        ...
        // m>0
        for (Integer m=1; m<=_Alpha_.lastRow(); ++m) {
            T tmp = 0.;
            for (int i=0; i<=r; ++i) {
                tmp += binomial(r,i)*powii(m,i)*_Alpha_(0,r-i);
            }
...
MRA<T,Primal,Interval,DKU>::_beta_()
{
    ...
    T factor = rsqrt2<T>();
    for (int r=0; r<d; ++r) {
        for (Integer m=_Beta_.firstRow(); m<=_Beta_.lastRow(); ++m) {
            T tmp = 0.;
...
MRA<T,Primal,Interval,DKU>::_calcM0()
{
    ...
    for (int r=0; r<d; ++r) {
        Left(l-d+r, l-d+r) = rsqrt2<T>() * pow2i<T>(-r);
    }

    for (int m=l; m<=2*l+l1-1; ++m) {
        for (int k=0; k<d; ++k) {
            Left(m,l-d+k) = rsqrt2<T>() * pow2i<T>(-k) * _Alpha_(m,k);
        }
    }
    ...
    BSpline<T,Primal,R,CDF> phi(d);
    for (Integer c=l, r=2*l+l1; c<=pow2i<T>(min_j0)-q-1; ++c, r+=2) {
        Mj0(_(r,r+phi.a.length()-1),c) = rsqrt2<T>() * phi.a; 
    }
...
#endif // LAWA_CONSTRUCTIONS_INTERVAL_DKU_PRIMAL_MRA_TCC
