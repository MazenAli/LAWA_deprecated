#ifndef LAWA_CONSTRUCTIONS_INTERVAL_DKU_DUAL_MRA_TCC
#define LAWA_CONSTRUCTIONS_INTERVAL_DKU_DUAL_MRA_TCC 1
...
{
    assert(j>=min_j0);
    return Range<Integer>(l_-d_+_bc(0), pow2i<T>(j)-l_-mu+d_-_bc(1));
}
...
{
    return Range<Integer>(l_-d_+_bc(0),l_-1);
}
...
{
    assert(j>=min_j0);
    return Range<Integer>(l_, pow2i<T>(j)-l_-mu);
}
...
{
    assert(j>=min_j0);
    return Range<Integer>(pow2i<T>(j)-l_-mu+1, pow2i<T>(j)-l_-mu+d_-_bc(1));
}
...
MRA<T,Dual,Interval,DKU>::_alpha()
{
    ...
    for (int r=1; r<d_; ++r) {
        // m<0
        for (Integer m=_Alpha.firstRow(); m<0; ++m) {
            T tmp = 0.;
            for (int i=0; i<=r; ++i) {
        ...
        // m>0
        for (Integer m=1; m<=_Alpha.lastRow(); ++m) {
            T tmp = 0.;
            for (int i=0; i<=r; ++i) {
                tmp += binomial(r,i)*powii(m,i)*_Alpha(0,r-i);
            }
...
MRA<T,Dual,Interval,DKU>::_beta()
{
    ...
    T factor = rsqrt2<T>();
    for (int r=0; r<d_; ++r) {
        for (Integer m=_Beta.firstRow(); m<=_Beta.lastRow(); ++m) {
            T tmp = 0.;
...
MRA<T,Dual,Interval,DKU>::_calcM0_()
{
    ...
    for (int m=low; m<=hi; ++m) {
        ML_c(m,m) = rsqrt2<T>() * pow2i<T>(-m+low);
    }
    for (int m=hi+1; m<=2*l_+l1_-1; ++m) {
        for (int k=low; k<=hi; ++k) {
            ML_c(m,k) = rsqrt2<T>() * pow2i<T>(-k+low) * _Alpha(m,k-low);
        }
    }
    ...
    BSpline<T,Dual,R,CDF> phi_(d,d_);
    for (Integer c=l_, r=2*l_+l1_; c<=pow2i<T>(min_j0)-q_-1; ++c, r+=2) {
        Mj0_(_(r,r+phi_.a_.length()-1),c) = rsqrt2<T>() * phi_.a_; 
    }
...
#endif // LAWA_CONSTRUCTIONS_INTERVAL_DKU_DUAL_MRA_TCC
