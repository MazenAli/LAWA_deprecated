    ...
    : public BasisFunction<_T,Primal,Periodic,CDF>
{
    ...
    T
    operator()(T x, int j, Integer k, unsigned short deriv) const;    // (cumulated) evaluation on [0,1]

    PeriodicSupport<T>
    support(int j, Integer k) const;            // subset of [0,1], may be divided in 2 parts

    DenseVector<Array<T> >
    singularSupport(int j, Integer k) const;    // Points in [0,1]
