...
{
    ct_assert(IsRealline<Basis1D>::value);

    typedef CompressionPDE1D<T, Basis1D>                             Compression1D;

    typedef NoPreconditioner<T,Index1D>                              NoPreconditioner1D;

    typedef LaplaceOperator1D<T, Basis1D>                            LaplaceOperator1D;

    typedef MapMatrix<T, Index1D, LaplaceOperator1D,
                     Compression1D, NoPreconditioner1D>              DataLaplace1D;


    AdaptiveLaplaceOperator1D(const Basis1D &_basis1d, T _c, T thresh=0.,
