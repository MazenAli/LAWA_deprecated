...
T
f(T x) { return std::exp(-5.L*(x-0.5L)*(x-0.5)); }
...
int main(int argc, char*argv[])
{
    ...
    /// In case of periodic functions, we have to represent them first in a B-spline basis
    /// (refinement basis).
    ...
    ofstream plotfile("test_periodic.txt");
    ...
    /// wavelets are used!!
    ofstream plotfile2("test2_periodic.txt");
    ...
    plotCoeff(u, basis, "coeff_multi_scale_periodic", false, true);
    plotCoeff(u_loc_single, basis, "coeff_local_single_scale_periodic", true, true);
