...
typedef LaplaceOperator1D_PG<T,PrimalBasis, SecondBasis>            BilinearForm;
typedef LaplaceOperator1D_PG<T,RefinementBasis,
							SecondRefinementBasis>  				RefinementBilinearForm;
...
void
computeEvalURef(const BilinearForm &Bil, const TreeCoefficients1D<T> &v_tree, TreeCoefficients1D<T> &Uv_tree);
...
int main(int argc, char*argv[])
{
    ...
    BilinearForm            Bil(basis, secondbasis);
    RefinementBilinearForm  RefineBil(refinementbasis, secondrefinementbasis);

    //LocOp1D localOperator1D(basis,basis,refinementbasis.LaplaceOp1D);
    LocOp1D localOperator1D(secondbasis,basis, RefineBil);

    stringstream ct_filename;
    ct_filename << "comptime_locOp1d_" << d << "_" << j0 << "_" << J << "_pg.dat";
    ...
    for (int j=j0; j<=J; ++j) {
        ...
        cout << "N = " << N << endl;
        time.start();
        computeEvalARef(Bil, v_tree, Av_ref_tree);
        time.stop();
...
//        cout << "Av_tree = " << Av_tree << endl;
        ...
        time.start();
        computeEvalURef(Bil, v_tree, Uv_ref_tree);
        time.stop();
        time.start();
...
//        cout << "Uv_tree = " << Uv_tree << endl;
        ...
        time.start();
        computeEvalLRef(Bil, v_tree, Lv_ref_tree);
        time.stop();
        time.start();
