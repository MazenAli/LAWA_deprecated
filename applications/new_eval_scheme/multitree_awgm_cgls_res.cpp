...
//                                  bool diffusionIsZero=false);
    ...
    for(int iter = 0; iter <= 100; ++iter){
		...
		// Compute u_leafs (using that p is still supported on the previous index set)
        std::cerr << "      Computing u_leafs... " << std::endl;
		u_leafs.clear();
        for (Coefficients<Lexicographical,T,Index2D>::const_iterator it=u.begin(); it!=u.end(); ++it) {
            if (p.find((*it).first)==p.end()) u_leafs[(*it).first] = 0.;
        }
        std::cerr << "      ... finished. Size of u_leafs: " << u_leafs.size() << std::endl;
