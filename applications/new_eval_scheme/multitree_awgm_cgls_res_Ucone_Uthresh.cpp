...
//                                  bool diffusionIsZero=false);
	...
	*/
    ...
    for(int iter = 0; iter <= 100; ++iter){
		...
		IndexSet<Index2D>						Cdiff_u;				 // new indizes in cone
        ...
        // THRESH(u)
		Coefficients<Lexicographical,T,Index2D> u_thresh(SIZEHASHINDEX2D);
		// First do thresh on u
		// add buckets to dummy vector (Ap)
		long double unorm = u.norm((T)2.);
		T u_threshbound = std::sqrt(1-awgm_u_alpha*awgm_u_alpha) * unorm/std::sqrt(T(u.size()));
		std::cerr << "      ||u||_2 : " << unorm << ", Threshbound u: " << u_threshbound << std::endl;
		Coefficients<Bucket,T,Index2D> u_bucket;
		u_bucket.bucketsort(u, u_threshbound);

		long double P_u_square = 0.0L;
		for (int i=0; i<(int)u_bucket.bucket_ell2norms.size(); ++i) {
			P_u_square += std::pow(u_bucket.bucket_ell2norms[i],2.0L);
			cerr << "     Bucket " << i << ": L2-norm " << u_bucket.bucket_ell2norms[i] << endl;
			u_bucket.addBucketToCoefficients(u_thresh,i);
			if (P_u_square >= awgm_u_alpha*unorm*awgm_u_alpha*unorm) {
				break;
			}
		}
		cout << "     ||u_thresh||_2: " << u_thresh.norm((T)2.) << endl;

		// Cone around u_thresh
		extendMultiTree(basis2d_trial, u_thresh, resNE, Cdiff_u, "standard", false, true);

		u = u_thresh;
		LambdaTrial = supp(u);
		std::cerr << "           #supp u_thresh = " << u.size() << std::endl;
        ...
        // Cone around u
        extendMultiTree(basis2d_trial, u, resNE, Cdiff_u, "standard", false, true);
        time.stop();
        ...
        time.stop();
        std::cerr << "         ... finished after " << time.elapsed() << std::endl;
        std::cerr << "          Cdiff_u = " << Cdiff_u.size() << std::endl;
