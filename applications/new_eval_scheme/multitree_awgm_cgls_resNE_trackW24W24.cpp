    ...
    for(int iter = 0; iter <= 100; ++iter){
		...
		if (resNE.size()!=0) {
            ...
            std::cerr << "         ||P_{Lambda}r ||_2 = " << std::sqrt(P_Lambda_Residual_square)
                      << ", alpha*Residual_NE = " << awgm_alpha*Residual_NE << std::endl;
		...
		// Complete vector r
        std::cerr << "      Size of LambdaTrial before extension: " << LambdaTrial.size() << std::endl;
        std::cerr << "      Size of extension set: " << p.size() << std::endl;
        IndexSet<Index2D> LambdaTrialDiff;
		for (Coefficients<Lexicographical,T,Index2D>::const_iterator it=p.begin(); it!=p.end(); ++it) {
			if (u.find((*it).first)==u.end()) {
				completeMultiTree(basis2d_trial, (*it).first, u, LambdaTrialDiff, 0, true);
			}
		}
		LambdaTrial = supp(u);
        std::cerr << "      Size of multitree extensions: " << LambdaTrialDiff.size() << LambdaTrialDiff << std::endl;
        std::cerr << "      Size of LambdaTrial after extension: " << LambdaTrial.size() << std::endl;

        //-------
        if(LambdaTrial.find(tracked_ind)!=LambdaTrial.end()){
        	cout << "INDEX W(2,4)xW(2,4) in LambdaTrial." << endl;
        }
        else{
        	cout << "INDEX W(2,4)xW(2,4) NOT in LambdaTrial." << endl;
        }
        //-------
