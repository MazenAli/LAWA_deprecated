...
int main () {
	...
	*/
    ...
    awgm_u.awgm_params.tol = 5e-03 * 1e-3;
    awgm_u.set_initial_indexset(Lambda);
    awgm_u.awgm_params.max_basissize = 400000;
    awgm_u.awgm_params.clear_solver = true;
    ...
    awgm_rieszF.awgm_params.tol = 5e-04 * 1e-3;
    awgm_rieszF.awgm_params.info_filename = "stempel_rieszF_conv_info.txt";
    awgm_rieszF.awgm_params.max_basissize = 400000;
    awgm_rieszF.awgm_params.clear_solver = true;
    ...
    awgm_rieszA.awgm_params.tol = 5e-04 * 1e-3;
    awgm_rieszA.awgm_params.info_filename = "stempel_rieszA_conv_info.txt";
    awgm_rieszA.awgm_params.max_basissize = 400000;
    awgm_rieszA.awgm_params.clear_solver = true;
    ...
    awgm_rieszRes.awgm_params.tol = 5e-04 * 1e-3;
    awgm_rieszRes.awgm_params.print_info = false;
    awgm_rieszRes.awgm_params.clear_solver = true;
...
*/
   ...
   */ 
    ...
    /*
    int old_N = 1;
    rb_system.read_rb_data("Runs/Runs_NxD/Run7/training_data_stempel", old_N);
    rb_base.read_basisfunctions("Runs/Runs_NxD/Run7/training_data_stempel/bf", old_N);
    rb_base.read_rieszrepresentors("Runs/Runs_NxD/Run7/training_data_stempel/representors", old_N);
    rb_base.read_greedy_info("Runs/Runs_NxD/Run7/stempel_greedy_info.txt", old_N);
	*/

    int old_N = 14;
    rb_system.read_rb_data("../Run1b_all/training_data_stempel", old_N);
    rb_base.read_basisfunctions("../Run1b_all/training_data_stempel/bf", old_N);
    rb_base.read_rieszrepresentors("../Run1b_all/training_data_stempel/representors", old_N);
    rb_base.read_greedy_info("../Run1b_all/stempel_greedy_info.txt", old_N);
    //rb_base.read_repr_accuracies("../Run7c/stempel_greedy_info.txt_repr_accuracies ", old_N);
    

    /*
    ParamType mu = {{0.01, 0.789474}};
    DataType res_repr;
    DenseVectorT u_N = rb_system.get_rb_solution(1,mu);

    awgm_rieszRes.awgm_params.tol = 1.449918329100e-04;
    cout << "Parameters Riesz Solver Res : " << std::endl << std::endl;
    awgm_rieszRes.awgm_params.print();

    T errbound = rb_base.get_direct_errorbound(u_N, mu, res_repr);
	*/

    rb_base.train_Greedy(old_N);
