...
int main () {
	...
	*/
    ...
    affine_lhs.set_param(p);
    affine_rhs.set_param(p);
    affineA_rhs.set_param(p);

    CoeffVector u,v,b_u_v_lhs;
    FillWithZeros(LambdaTest, b_u_v_lhs);
    for(auto& ind : LambdaTrial){
    	completeMultiTree(basis2d_trial, ind, u, 0,1);
    	u[ind] = 1.;

    	affine_lhs.eval(u, b_u_v_lhs);
    	CoeffVector f_v_rhs = affine_rhs(LambdaTest);

    	affineA_rhs.set_active_u(&u);
    	rieszRes_rhs.set_active_u(&u);

    	CoeffVector b_u_v_rhs = affineA_rhs(LambdaTest);
    	CoeffVector direct_res = rieszRes_rhs(LambdaTest);

    	f_v_rhs -= b_u_v_lhs;
    	f_v_rhs -= direct_res;
    	b_u_v_lhs -= b_u_v_rhs;
    	cout << "Size u = " << u.size() << ", Diff B: " << b_u_v_lhs.norm() << ", Diff Res: " << f_v_rhs.norm() << endl;
    }
