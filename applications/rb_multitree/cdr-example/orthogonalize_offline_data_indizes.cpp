...
int main (int argc, char* argv[]) {
    ...
    if(argc != 3){
    	cerr << "Usage: " << argv[0] << " offline_data_folder output_data_folder" << endl;
    	exit(1);
     }

    string offline_folder = argv[1];
    string output_folder = argv[2];
    vector<int> indizes = {{4,1,2,3}};
    int N = indizes.size();
	...
	*/
    ...
    DataType& bf_ONB = rb_base.rb_basisfunctions[indizes[0]];
    FillWithZeros(Lambda_union, bf_ONB);
    RB_ONB.push_back(bf_ONB);

    for(int n = 1; n < N; ++n){
        FillWithZeros(Lambda_union, rb_base.rb_basisfunctions[indizes[n]]);
        DataType new_bf = rb_base.rb_basisfunctions[indizes[n]];
        for(auto& bf : RB_ONB){
    		new_bf = new_bf - bf * rb_truth.innprod_Y_u_u(bf, rb_base.rb_basisfunctions[indizes[n]]);
    	}        
