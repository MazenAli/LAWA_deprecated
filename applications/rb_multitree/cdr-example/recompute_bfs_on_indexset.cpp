...
int main (int argc, char* argv[]) {
    ...
    if(argc != 2){
    	cerr << "Usage: " << argv[0] << "output_data_folder" << endl;
    	exit(1);
     }
	...
	*/
    ...
    ParamType mu = {{0, -9}};
    rb_system.read_rb_data("Runs/Stage7/Run3/training_data_stage7", 6);
    rb_base.read_basisfunctions("Runs/Stage7/Run3/training_data_stage7/bf", 6);
    //rb_base.read_rieszrepresentors("Runs/Stage7/Run3/training_data_stage7/representors", 6);
    //rb_base.read_greedy_info("Runs/Stage7/Run3/awgm_stage7_greedy_info.txt", 6);
    
    
    IndexSet<Index2D> Lambda_BF1 = supp(rb_base.rb_basisfunctions[0]);
    IndexSet<Index2D> Lambda_BF5 = supp(rb_base.rb_basisfunctions[4]);
    
    DataType u_bf1, u_bf5;
    FillWithZeros(Lambda_BF1, u_bf1);
    rb_truth.get_truth_solution(mu, u_bf1);
    string filename_bf1 = output_folder+"/u_bf1.txt";
    saveCoeffVector2D(u_bf1,basis2d_trial, filename_bf1.c_str());
    
    
    FillWithZeros(Lambda_BF5, u_bf5);
    rb_truth.get_truth_solution(mu, u_bf5);
    string filename_bf5 = output_folder+"/u_bf5.txt";
    saveCoeffVector2D(u_bf5,basis2d_trial, filename_bf5.c_str());
