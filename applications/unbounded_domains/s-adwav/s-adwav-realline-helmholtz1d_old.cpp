...
const FunctionSide side = Primal;
...
typedef DiagonalMatrixPreconditioner1D<T,Basis1D,HelmholtzBilinearForm1D>  Preconditioner1D;
...
int main (int argc, char *argv[]) {
    ...
    RhsIntegral1D rhsintegral1d(basis, rhs, refsol.deltas, 40);
    Rhs F(rhsintegral1d,P);

    IndexSet<Index1D> InitialLambda;
    InitialLambda.insert(Index1D(jmin,1,XBSpline));

    S_Adwav s_adwav(basis, A, F, contraction, threshTol, cgTol, resTol, NumOfIterations, 2, 1e-7, 100000);
    cout << "... finished." << endl;
