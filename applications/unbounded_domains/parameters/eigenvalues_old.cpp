...
typedef IdentityOperator1D<T, Basis1D>                            HelmholtzBilinearForm1D;
...
typedef DiagonalMatrixPreconditioner1D<T,Basis1D,
                                       HelmholtzBilinearForm1D>   Preconditioner1D;
...
int main (int argc, char *argv[]) {
    ...
    HelmholtzBilinearForm1D Bil(basis);
    Preconditioner1D P(Bil);
...
LambdaForEigenvalues(int jmin, int jmax, const Basis1D &basis, T radius, bool with_BSplines)
{
    ...
    if (with_BSplines) {
        for (int j=jmin; j<=jmax; ++j) {
            ...
            //cout << "k_left = " << k_left << ", k_right = " << k_right << endl;
            for (int k=k_left; k<=k_right; ++k) {
                Lambda.insert(Index1D(j,k,XWavelet));
            }
        }

        k_left  = -30;//int(std::floor(-pow2i<T>(jmin)*radius-phi.support(0,0).l2));
        k_right = 30;//int(std::ceil(  pow2i<T>(jmin)*radius-phi.support(0,0).l1));
        for (int k=k_left; k<=k_right; ++k) {
