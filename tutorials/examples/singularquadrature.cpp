...
int main()
{
    ...
    cout << "Non-singular, non-quadratic: approx = " << approx << ", ref_val = " << ref_val
         << ", diff = " << approx-ref_val << endl;


    // Corner singularity in (a1,b2), quadratic domain
    ref_val = (-217. + log(256.))/4096.;
    approx  = singularIntegralPP(0.75,1.,0.5,0.75);
    cout << "Corner singularity (a1,b2), quadratic: approx = " << approx << ", ref_val = " << ref_val
         << ", diff = " << approx-ref_val << endl;
    ...
    cout << "Corner singularity (a1,b2), non-quadratic1: approx = " << approx << ", ref_val = " << ref_val
         << ", diff = " << approx-ref_val << endl;
    ...
    cout << "Corner singularity (a1,b2), non-quadratic2: approx = " << approx << ", ref_val = " << ref_val
         << ", diff = " << approx-ref_val << endl;

    // Corner singularity in (b1,a2), quadratic domain
    ref_val = (-97.-52.*log(2.))/65536.;
    approx  = singularIntegralPP(0.125,0.25,0.25,0.375);
    cout << "Corner singularity (b1,a2), quadratic: approx = " << approx << ", ref_val = " << ref_val
         << ", diff = " << approx-ref_val << endl;

    // Corner singularity in (b1,a2) non-quadratic domain 1
    ref_val = -3.*(272. + 1560.*log(2.) + 1200.*log(3.) - 1323*log(7.))/32768.;
    approx  = singularIntegralPP(0.125,0.25,0.25,1.);
    cout << "Corner singularity (b1,a2), non-quadratic: approx = " << approx << ", ref_val = " << ref_val
         << ", diff = " << approx-ref_val << endl;

    // Corner singularity in (b1,a2) non-quadratic domain 2
    ref_val = (-3065. + 5040.*log(8./5.) - 15972.*log(2.) + 4608*log(3.) + 2590.*log(5.))/65536.;
    approx  = singularIntegralPP(0.125,0.75,0.75,0.875);
    cout << "Corner singularity (b1,a2), non-quadratic 2: approx = " << approx << ", ref_val = " << ref_val
         << ", diff = " << approx-ref_val << endl;


    // Diagonal singularity in quadratic domain
    ref_val = (-10225. - 10196*log(2.) + 4608.*log(3.))/65536.;
    approx  = singularIntegralPP(0.125,0.75,0.25,0.875);
    cout << "Diagonal singularity, quadratic: approx = " << approx << ", ref_val = " << ref_val
         << ", diff = " << approx-ref_val << endl;

    // Diagonal singularity in non-quadratic domain
    ref_val = (-335. - 764.*log(2.) + 576.*log(3.))/8192.;
    approx  = singularIntegralPP(0.125,0.375,0.25,0.875);
    cout << "Diagonal singularity, non-quadratic: approx = " << approx << ", ref_val = " << ref_val
         << ", diff = " << approx-ref_val << endl;
    ...
    ref_val2 = (-100. + 28.*log(2.) - 3.*log(16.))/64.;
